...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
    vec2 uv = screen_coords / resolution; // 规范化屏幕坐标 (0,1)
    float aspectRatio = resolution.x / resolution.y; // 计算高宽比
    ...
    for (int i = 0; i < numParticles; ++i) {
        ...
        vec2 particlePosition = initialPosition + velocity * time; // 更新粒子位置
        particlePosition = mod(particlePosition, vec2(1.0)); // 保证粒子位置在 [0, 1] 范围内循环
