...
function state_machine_UI_start_scene_config_controller(obj,input_id)
    local switch = {
        ...
        ["off_state"] = function() 
            local flash_in_anim = anim_UI_point_linear_start_scene_general_flash_in_0p5_1_opacity
            ...
            if INPUT_SYS_CURRENT_COMMAND_STATE[input_id]["D"] == "Pressing" or INPUT_SYS_CURRENT_COMMAND_STATE[input_id]["D"] == "Holding" then
                -- 设置flash in 动画
                obj[4] = 0.5
                init_point_linear_anim_with(obj,flash_in_anim)
                obj["state"] = "flash_in"
            end
        end,
        ["flash_in"] = function() 
            local flash_in_anim = anim_UI_point_linear_start_scene_general_flash_in_0p5_1_opacity
            local flash_out_anim = anim_UI_point_linear_start_scene_general_flash_out_1_0p5_opacity
            point_linear_animator(obj,flash_in_anim)
            if INPUT_SYS_CURRENT_COMMAND_STATE[input_id]["D"] == "Releasing" or INPUT_SYS_CURRENT_COMMAND_STATE[input_id]["D"] == "Released" then
                -- 设置flash out 动画
                obj[4] = 1
                init_point_linear_anim_with(obj,flash_out_anim)
                obj["state"] = "flash_out"
            elseif get_point_linear_anim_end_state(obj,flash_in_anim) then
                obj[4] = 1
                obj["state"] = "on_state"
            end
        end,
        ...
        ["flash_out"] = function() 
            local flash_in_anim = anim_UI_point_linear_start_scene_general_flash_in_0p5_1_opacity
            local flash_out_anim = anim_UI_point_linear_start_scene_general_flash_out_1_0p5_opacity
            point_linear_animator(obj,flash_out_anim)
            if INPUT_SYS_CURRENT_COMMAND_STATE[input_id]["D"] == "Pressing" or INPUT_SYS_CURRENT_COMMAND_STATE[input_id]["D"] == "Holding" then
                -- 设置flash out 动画
                obj[4] = 0.5
                init_point_linear_anim_with(obj,flash_in_anim)
                obj["state"] = "flash_in"
            elseif get_point_linear_anim_end_state(obj,flash_out_anim) then
                obj[4] = 0.5
                obj["state"] = "off_state"
            end
        end,
